...
LoadBalancer extends NodePort by provisioning an external load balancer through your cloud provider. When you create a LoadBalancer Service, the cloud controller manager creates a load balancer (like an AWS ELB or GCP Load Balancer) that routes traffic to the NodePort, which then routes to your pods.
...
For advanced traffic management, mutual TLS, and observability, consider integrating a service mesh like Istio or Linkerd.

### Linkerd: Lightweight Service Mesh

Linkerd provides automatic mTLS, traffic splitting, and observability with minimal configuration:
...
Istio provides comprehensive traffic management, security, and observability:
...
Use connection pooling in your application code to reduce the overhead of establishing new connections through the Service proxy:
...
For latency-sensitive applications, use Topology Aware Routing to keep traffic within the same availability zone:
...
The Gateway API is the successor to Ingress, providing a more expressive, role-oriented model. It introduces three resource types:

- **GatewayClass**: Defines the infrastructure provider (similar to StorageClass)
- **Gateway**: Cluster operator configures which ports, protocols, and certificates to use
- **HTTPRoute/GRPCRoute/TLSRoute/TCPRoute**: Application developers define routing rules
...
- **Role separation**: Infrastructure team manages Gateways, application team manages Routes
- **Protocol support**: Native support for gRPC, TCP, UDP, and TLS
- **Header-based routing**: Match on headers, query params, and method
- **Traffic splitting**: Built-in support for weighted backends
- **Cross-namespace references**: Routes can reference Gateways in other namespaces
...
kubectl run debug-prod --image=busybox --rm -it --restart=Never -n production -- sh
...
|---------|--------|----------|
| Using `hostNetwork: true` on pods | Port conflicts and security exposure | Use Services and Ingress for external access |
| Not setting `externalTrafficPolicy: Local` | Loss of client source IP | Set `externalTrafficPolicy: Local` on LoadBalancer Services |
| Ingress without Ingress Controller | Ingress resources have no effect | Always install an Ingress Controller first |
| DNS caching in applications | Stale endpoint resolution | Set appropriate TTL and use headless Services |
| Missing NetworkPolicies | Unrestricted pod-to-pod communication | Implement default deny policies |
| Overriding cluster DNS | Service discovery failures | Use `dnsPolicy: ClusterFirst` (default) |
| Too many LoadBalancer Services | High cloud costs | Use Ingress to consolidate external access |
| Ignoring CoreDNS performance | DNS resolution delays | Scale CoreDNS replicas for large clusters |
...
3. **Set resource requests and limits**: Ingress controllers consume CPU and memory
4. **Use cert-manager for TLS**: Automates issuance, renewal, and secret management
5. **Monitor kube-proxy and CoreDNS**: Set up alerts for restarts and latency spikes
6. **Prefer Ingress over LoadBalancer**: Consolidate external access points
7. **Enable topology-aware routing**: Reduce cross-zone latency and costs
8. **Document DNS naming conventions**: Maintain service discovery documentation
9. **Use Gateway API for new deployments**: More expressive than Ingress
10. **Implement service mesh gradually**: Start with mTLS, add traffic management as needed
