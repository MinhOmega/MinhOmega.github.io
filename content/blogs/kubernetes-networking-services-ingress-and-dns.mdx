...
Kubernetes implements a flat networking model where every pod gets its own IP address, and any pod can communicate with any other pod without NAT. This model is achieved through the Container Network Interface (CNI), which handles pod-to-pod networking across nodes. Popular CNI implementations include Calico, Flannel, Cilium, and Weave, each offering different features like network policies, encryption, and observability.

The Kubernetes networking model has three primary concerns: container-to-container communication within a pod (handled by the pod's network namespace), pod-to-pod communication across nodes (handled by the CNI plugin), and external-to-internal communication (handled by Services and Ingress). Each layer builds on the previous one, creating a comprehensive networking stack.

Pods are assigned IP addresses from a pod CIDR range configured on each node. When a pod is deleted and rescheduled, it receives a new IP address. This ephemeral nature is precisely why Services exist—they provide stable virtual IPs that persist regardless of pod lifecycle events.
...
DNS resolution in Kubernetes is handled by CoreDNS, which runs as a Deployment in the kube-system namespace. CoreDNS watches the Kubernetes API for new Services and Pods, automatically creating DNS records that enable service discovery by name rather than IP address. This DNS-based discovery is the primary mechanism applications use to find and communicate with other services in the cluster.
