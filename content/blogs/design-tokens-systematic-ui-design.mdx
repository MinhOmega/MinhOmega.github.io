...
Design tokens enable theming by defining a set of semantic tokens that reference different primitive values depending on the active theme. The semantic layer abstracts intent from implementation: instead of using `color-gray-900` directly, you use `color-text-primary` that resolves to `color-gray-900` in the light theme and `color-gray-100` in the dark theme. This separation means component code never changes when themes switch — only the token values change.
