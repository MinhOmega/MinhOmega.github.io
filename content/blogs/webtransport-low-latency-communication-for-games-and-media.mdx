...
Raw UDP gives you speed but no reliability, ordering, or congestion control. QUIC adds these features at the transport layer while preserving UDP's key advantage: no head-of-line blocking between independent streams. In TCP, if packet 5 is lost, packets 6-100 are all blocked until packet 5 is retransmitted. In QUIC, each stream is independently reliable—a lost packet on stream A doesn't block stream B.
