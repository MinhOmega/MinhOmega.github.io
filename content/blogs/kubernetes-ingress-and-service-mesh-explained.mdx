...
The Kubernetes Gateway API is the successor to Ingress, providing a more expressive and role-oriented approach to traffic management. It separates infrastructure concerns (GatewayClass) from cluster operations (Gateway) from application routing (HTTPRoute). All major Ingress controllers and service meshes are adopting Gateway API.

Istio's ambient mesh mode eliminates sidecar proxies, reducing resource overhead and operational complexity. This moves proxy functionality to ztunnel (a per-node daemon) for L4 and waypoint proxies for L7, making service mesh adoption significantly easier.
